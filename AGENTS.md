# TG Slides Template — AI Instructions

## Project Overview

**Next.js + MDX slide deck system** branded to the Taylor Geospatial brand guide. Warm dark theme (brown/ivory/periwinkle/red).
...
From `theme/colors.ts` (sourced from TG Brand Guide):
...
Cover/closing slide with TG logo + gradient title.
...
Standard fenced Markdown code blocks — `CodeBlock` is applied automatically. Syntax theme uses TG brand colors. Blocks have a gradient accent stripe (periwinkle→red) on the left edge.
...
TG brandmark SVG + "Taylor Geospatial" wordmark. Included automatically in `TitleSlide`.
...
| `components/Logo.tsx` | TG brandmark + wordmark |
